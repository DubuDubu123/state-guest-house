...
">{{$booking->user->address}} </span></p>
                                ...
                                <div class="bio-row">
                                    <p><span>Check Out Date</span><span class="value-data">: <?php echo date('Y-m-d', strtotime($booking->checkout_date)); ?></span></p>
                                </div>
                                @if($booking->status > 0)
                                @if($booking->status >= 1)
                                <div class="bio-row">
                                    <p><span>Check In Time</span><span class="value-data">: <?php echo date('Y-m-d H:i:s A', strtotime($booking->actual_checkin_date)); ?></span></p>
                                </div>
                                @endif
                                @if($booking->status >= 2)
                                <div class="bio-row">
                                ...
                                @endif
                                @elseif($date_diff > 1 && $booking->status == 0) 
                ...
                swal({
                    ...
                    <div class="text-center m-b-0" style="/* background: #0A7E8C; */">
                                <button class="generate_invoice btn btn-custom waves-effect waves-light make-confirm" data-url="{{url('/')}}/types/confirm-checkout/{{$booking->id}}" type="button" style="background: #0A7E8C;
                                        padding-left: 30px;padding-right: 30px;">Generate Invoice</button>
                                </div> 
                                                                </div>
                                                                </div>
                                `);
...
$(document).on('click', '.sweet-delete1', function(e) {
                ...
                swal({
                    ...
                    <label for="image" style="font-size: 16px;"> Select Checkin Date  </label><br> 
                    <input type="hidden" name="id" value="{{$booking->id}}">
                    <input type="date" id="date" name="date" value="<?php echo date('Y-m-d', strtotime($booking->checkin_date)); ?>" style="width: 100%;"></div><div class="form-group" style="margin-top: 20px;"><label for="image" style="font-size: 16px;">Checkout Date</label><br> <input type="text" style="width: 100%;" disabled="" value="<?php echo date('Y-m-d', strtotime($booking->checkout_date)); ?>"></div> 
...
<br> 
    ...
    <label for="image" style=" font-size: 16px;
                                ...
                                <div class="text-center m-b-0" style="/* background: #0A7E8C; */">
                                <button class="confimr btn btn-custom waves-effect waves-light make-confirm" data-url="{{url('/')}}/types/confirm-checkin/{{$booking->id}}" type="button" style="background: #0A7E8C;
                                        padding-left: 30px;padding-right: 30px;color:white">Confirm</button>
                                </div>
                            <br> 
                                            </div>
                                            </form>
                        ...
                        $('#room-tariff').on('change', 'select', function() {
                            // Get the selected value and perform actions as needed
                            var selectedValue = $(this).val();
                            console.log('Selected value:', selectedValue);

                            var form_data = new FormData($("#confirm-checkin-booking")[0]);
                            form_data.append("eta",1);
                            var url = "{{url('/')}}/types/confirm-checkin/{{$booking->id}}";
                            $.ajax({
                                    url: url,
                                    type:'post',
                                    data: form_data,
                                    ...
                                    processData: false, // Tells jQuery to process the data (default: true) 
                                    success: function(res) { 
                                        $(".membership-price").html(`₹ ${res.data}`);
                                    console.log(res); 
                                    }
                                }); 
            ...
            function formatDateToYMD1(date) {
            ...
            $('#date').attr('max', formatDateToYMD1(checkoutDate));
