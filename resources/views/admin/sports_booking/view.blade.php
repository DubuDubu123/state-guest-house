...
.alert-success, .callout.callout-success, .label-success, .modal-success .modal-body{
    background-color: #09AAB8 !important;
}
    button.btn.btn-primary.btn-sm.m-5.book-now.active {
        cursor: pointer !important;
    }
...
.skin-blue-light .sidebar-menu>li.active>a {
    border-left-color: #09AAB8 !important;
}
.btn-primary {
...
">{{$booking->user->address}} </span></p>
                                ...
                                <div class="bio-row">
                                    <p><span>Subscription Type </span>: <span>{{ucfirst($booking->subscription_type)}}</span> </p>
                                </div>
                                <div class="bio-row">
                                    <p><span>Booked On </span><span class="value-data">:  <?php echo date('Y-m-d', strtotime($booking->created_at)); ?></span></p>
                                </div> 
                                <div class="bio-row">
                                    <p><span>Start Date</span>: <span> <?php echo date('Y-m-d', strtotime($booking->checkin_date)); ?></span> </p>
                                </div> 
                                <div class="bio-row">
                                    <p><span>End Date</span>: <span> <?php echo date('Y-m-d', strtotime($booking->checkout_date)); ?></span> </p>
                                </div> 
                           
                                <div class="bio-row">
                                    <p><span>Guest Type </span><span class="value-data">: {{ucfirst($booking->guest_type)}}</span></p>
                                </div>
                                <div class="bio-row">
                                    <p><span style="vertical-align: top;">Sports Name </span><span class="value-data">:
                                    @foreach($booking->details as $k=>$v)
                                     {{$v->tariff->name}}@if($k != (count($booking->details)-1)),@endif
                                    @endforeach
                                    </span>
                                    </p>
                                </div>
                                ...
                                <div class="bio-row" >
                                    ...
                                    @if($booking->status == 0)
                                    <span class="value-data" style=" background: #ff9900;display: inline;padding: 5px;border-radius: 3px;color: white;">Booked</span> 
                                    @else 
                                    <span class="value-data" style=" background: green;display: inline;padding: 5px;border-radius: 3px;color: white;">Completed</span>
                                    @endif
                                ...
                                <div class="bio-row" style="width:100%;">
                                    ...
                                    <span class="value-data" style=" background: green;display: inline;padding: 5px;border-radius: 3px;color: white;">Paid</span>
                                    @else
                                    <span class="value-data" style=" background: red;display: inline;padding: 5px;border-radius: 3px;color: white;">Not Paid</span>
                                    @endif
                                ...
                                @if($booking->status > 0)
                                <div class="form-group">
                                   <div class="col-12" style="text-align: right;  /* margin-right: 151px; */ position: relative;right: 30px; ">
                                   <a class="btn btn-success" style="font-size:16px;margin: auto;margin-top: 20px;" href="{{url("/")}}/sports-booking/view-invoice/{{$booking->id}}">
                                        Generate Invoice 
                                    </a>
                                    </div>
                                </div> 
...
$(document).on('click', '.sweet-delete', function(e) {
                ...
                swal({
                    ...
                    if (isConfirm) {
                        swal.close();  
                        $.ajax({
                            url: '{{url("/")}}/sports/cancel-booking/'+data_id+'', 
                            cache: false,
                            success: function(res) {
                                if(res.status)
                                {
            ...
            $(document).on('click', '.sweet-delete1', function(e) {
                ...
                swal({
                    ...
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                    confirmButtonText: "Payment Collected",
                    cancelButtonText: "No! Keep it",
                    closeOnConfirm: false,
                    closeOnCancel: true
                }, function(isConfirm) {
                    if (isConfirm) {
                        swal.close();  
                        $.ajax({
                            url: '{{url("/")}}/sports/confirm-booking/'+data_id+'', 
                            cache: false,
                            success: function(res) {
                                if(res.status)
                                {
