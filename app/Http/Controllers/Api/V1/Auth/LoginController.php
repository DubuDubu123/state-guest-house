...
class LoginController extends BaseLoginController
{
    ...
    public function validateDriver(GenericAppLoginRequest $request)
    {
                  
        $driver_exists = User::where('username', $request->username)->where('mobile', $request->mobile)->exists();

        $user = User::where('username', $request->username)->where('mobile', $request->mobile)->first();

            if($driver_exists &&  Hash::check($request->password, $user->password))
            {
               return $this->loginUserAccountApp($request, Role::DRIVER);
            }
        
         return $this->throwCustomException('Driver ID and Password doesn\'t match with the verified phone number');
