  ...
  public function register(Request $request)
  {
    ...
    $created_params['mobile'] = $validatedData['mobile'];
      ...
      if($request->has('refferal_code'))
      {
        ...
        $created_params['refferal_code'] = $request->input('refferal_code');

      } 

    $validate_exists_mobile = $this->user->belongsTorole([Role::DRIVER,Role::OWNER])->where('mobile', $request->input('mobile'))->exists();

    if ($validate_exists_mobile) {
        $this->throwCustomException('Provided mobile has already been taken');
    }
  
    $validate_already_registered = RegisteredDriver::where('mobile', $request->input('mobile'))->exists();

    if ($validate_already_registered) {
        $this->throwCustomException('Provided mobile has been already Registered Our Team will contact You');
    }

    RegisteredDriver::create($created_params);

   return $this->respondSuccess('Driver Registered');
    ...
    * @bodyParam mobile integer required mobile of driver
    ...
    public function validateDriverMobileForLogin(Request $request)
    {
        ...
        //     if ($validate_already_registered) {
        //         $this->throwCustomException('already registered');
        //     }
        // }
